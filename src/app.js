...
function updateProgress({ phase, current, total, percent }) {
  progress.value = percent;
  progressText.textContent = `${phase} — ${current}/${total} — ${percent}%`;
}
...
function render(result, dueDate, log) {
  ...
  for (const row of rows) {
    ...
    const invalidTag = row.TAG.length > 40;
    if (invalidTag) tr.classList.add('warning-row');
    for (const value of [row.Codigo, row['Tipo Inscrição'], row.Servico, row.TAG, row['Inscrição'], money(row.FGTS), money(row['FGTS Aprendiz'])]) {
...
processBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    render(currentResult, currentDueDate, currentLog);
    ...
    status.textContent = `Falha no processamento: ${error.message}`;
    console.error(error);
  } finally {
...
exportBtn.addEventListener('click', () => {
  try { exportWorkbook(currentResult, currentDueDate, currentLog); }
  catch (error) { status.textContent = `Falha ao gerar Excel: ${error.message}`; }
});
